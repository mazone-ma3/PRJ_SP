...
volatile __far unsigned char vs_count;
...
void set_sprite_locate(unsigned short pat_no, unsigned short index, unsigned char i,  unsigned short x, unsigned short y, unsigned short spr_x_size, unsigned short spr_y_size){
	unsigned short spr_size = SPR_SIZE(spr_x_size, spr_y_size);
	spram = (__far unsigned short  *)MK_FP(0xa000, ((SPR_ATR) + (pat_no * 8)));
...
void put_strings(int scr, int y, int x,  char *str, char pal)
{
	char chr;
	unsigned short i = 0;
	__far unsigned char *vram;
	unsigned char xx, yy;
	y = 28-y;

	vram = (__far unsigned char *)MK_FP(0x0c000, 0);
...
*/
//	spram = (__far unsigned char *)MK_FP(0xa000, SPR_ATR);
}
...
/* スタ－の表示(固定表示) */
	...
	i = STAR_NUM;
	while(i--){
		vram = MK_FP(0xa000, (star[0][i] + ((320 / STAR_NUM) >> 1) + 
			(star[1][i] * 80)) << 1);
		*vram |= star[4][i];
	}
...
//	_outb(0x443,scrl);
	...
	outportw(0x20c, scrl & 0x03ff);
	outportw(0x20e, (scrl * 160L) & 0xfffc);
	outportw(0x210, ((scrl * 160L) >> 16) & 0x0003);
	scrl += (200L * 1);
...
//			wait(1);
		...
		outportb(0x142, 0x81);	/* SPROV スプライト衝突判定 */
		while(inportb(0x142) & 0x05);
		if(inportb(0x146) & 0x20){

//		if(inportb(0x142) & 0x20){
			hit_check = TRUE;
		}else{
			hit_check = FALSE;
		}
...
//		_enable();
		bg_roll();
	}while((scrl_spd != 0) && (inportb(0x09) & 0x80));
...
//	while(inportb(0x0040) & 0x20));
	while((inportb(0x0142) & 0x40)); /* WAIT VSYNC */
	while(!(inportb(0x0142) & 0x40));
}
...
//	_FP_OFF(spram) = (1024 - MAX_SPRITE) * 8;
	spram = (__far unsigned short  *)MK_FP(0xa000, SPR_ATR); // + i * 8);
