...
//	outportw(0x100, 0xb062);	/* none-interless Graphic-on 400dot(200line) */
	...
	outportw(0x100, 0xbc62);	/* none-interless Graphic-on 400dot(200line) */
								/* 画面ON single-plane 2画面 */
//	outportw(0x100, 0xb021);

//	outportw(0x102, 0x0101);	/* graphic0/1 Width640 4dot/pixel */
// 	outportw(0x102, 0x1111);	/* graphic0/1 Width320 4dot/pixel */

//	outportb(0x153, 0x54);		/* G-VRAM選択 *
//	outportb(0x153, 0x51);		/* T-VRAM選択 */

//	outportw(0x106, 0xab89);	/* パレット指定画面割当て指定 */
//	outportw(0x106, 0xa89b);	/* パレット指定画面割当て指定 */
	outportw(0x106, 0xa89b);	/* パレット指定画面割当て指定 */
...
//	outportb(0x10d, 0x01);
//	outportw(0x10c, 0x0100);	/* カラーパレットモード (パレット0) */
//	outportw(0x10c, 0x01b0);	/* カラーパレットモード 混在/バレット1:GRP1 */
	outportw(0x10c, 0x01a0);	/* カラーパレットモード 混在/バレット1:GRP0 */

	outportw(0x124,0x00);	/* 透明色 GRP0 */
	outportw(0x126,0x01);	/* 透明色 GRP1 */
	outportw(0x12e,0x01);	/* 透明色 TEXT/SPRITE */
}
...
// 	outportw(0x100, 0xb020);	/* none-interless Graphic-on 400dot(400line) */
// 	outportw(0x102, 0x0101);	/* graphic0 Width640 4dot/pixel */
//	TVRAM_ON();
	outportw(0x106, 0xab89);	/* パレット指定画面割当て指定 */
	outportw(0x10c, 0x0180);	/* カラーパレットモード */
//	outportw(0x10c, 0x0110);	/* カラーパレットモード */
...
void paint(unsigned char color)
{
	...
	GVRAM_ON();
	__far unsigned short *vram;
	vram = (__far unsigned short *)MK_FP(0xa000, 0);

	for (i = 0; i < ((80 * 408L)); ++i){
		*(vram + i) = color + color *256;
	}
}
...
void paint2(unsigned char color)
{
	...
	GVRAM_ON();
	__far unsigned short *vram;
	vram = (__far unsigned short *)MK_FP(0xc000, 0);
...
void erase_allsprite(void)
{
	unsigned char i;
	TVRAM_ON();
	__far unsigned short  *spram;
	spram = (__far unsigned short *)MK_FP(0xa000, SPR_ATR);
...
{
//	char str[4];
	__far unsigned char *p0, *p1, *p2, *p3;
	p0 = (__far unsigned char *)MK_FP(0, (0x60 * 4));
	p1 = (__far unsigned char *)MK_FP(0, (0x60 * 4 + 1));
	p2 = (__far unsigned char *)MK_FP(0, (0x60 * 4 + 2));
	p3 = (__far unsigned char *)MK_FP(0, (0x60 * 4 + 3));
	__far unsigned char *pp0, *pp1, *pp2;
	pp0 = (__far unsigned char *)MK_FP( *p2 + *p3 * 256, *p0 + *p1 * 256 + 2);
	pp1 = (__far unsigned char *)MK_FP( *p2 + *p3 * 256, *p0 + *p1 * 256 + 3);
	pp2 = (__far unsigned char *)MK_FP( *p2 + *p3 * 256, *p0 + *p1 * 256 + 4);
