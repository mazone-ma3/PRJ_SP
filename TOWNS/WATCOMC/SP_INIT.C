...
//			palram[color] = green * 64 * 32 + red * 64 + blue * 2;
			break;
...
//			_FP_OFF(vram) = (j + i * 1024 + 32) / 2;
			vram = MK_FP(0x120, ((j + i * 1024 + 32) / 2));
			*vram = color;
		}
	}
}
...
char KYB_read( char mode, unsigned int *encode )
{
	int ret01 = 0;
/*
	rmi_call01.rmi_ds = segment01;
	rmi_call01.rmi_edi = 0x1000;
	rmi_call01.rmi_eax = (0x09 << 8) | mode;	// ah = 0x01;

	if (rmi_call(0x90, prot_addr01, segment01, &rmi_call01, &rmi_call01, 16) == 0) {
		if (((rmi_call01.rmi_eax >> 8) & 0xff) == 0) {
			*encode = rmi_call01.rmi_ebx;
		}

		if(!(rmi_call01.rmi_eax & 0x0f)){
			if( ((rmi_call01.rmi_edx >> 8) & 0x0f) != 255){
				ret01 = rmi_call01.rmi_edx & 0x0f;
			}
		}
	}
*/
...
char *SND_load(char *fn, char*SNDBUFF){
	...
	size = fread(SNDBUFF, SND_BUFFSIZE, 1, fp);
	fclose(fp);
...
//	MSV_init(MSVwork,MSVWorkSize,EXPMODE);
	if(SND_elevol_all_mute(-1))
		return ERROR;
	if(SND_elevol_mute(0x33))
		return ERROR;
...
//	if(error = SND_pcm_play( 71, 60, 127, SNDBUFF )){
...
//		wait_sprite();
		wait_vsync2();
	}while(1); //KYB_read( 1, &encode ) !=CODE_ESC);
